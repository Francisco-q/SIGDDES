...
const OpenMap: React.FC = () => {
  ...
  const handleCreateTotem = async (lat: number, lng: number) => {
    if (!["admin", "superuser"].includes(role as string)) {
      setWarningMessage("Solo administradores pueden crear tótems.");
  ...
  const handleCreateReception = async (lat: number, lng: number) => {
    if (!["admin", "superuser"].includes(role as string)) {
      setWarningMessage("Solo administradores pueden crear recepciones.");
      setWarningModalOpen(true);
      return;
    }
  ...
  const handleSavePoint = async (updatedPoint: TotemQR | ReceptionQR) => {
    if (!updatedPoint || !["admin", "superuser"].includes(role as string)) {
      setWarningMessage("Solo administradores pueden guardar puntos.");
      setWarningModalOpen(true);
      return;
    }
  ...
  const handleDeletePoint = async (pointId: number, isTotem: boolean) => {
    if (!["admin", "superuser"].includes(role as string)) {
      setWarningMessage("Solo administradores pueden eliminar puntos.");
      setWarningModalOpen(true);
      return;
    }
  ...
  const savePath = () => {
    if (!["admin", "superuser"].includes(role as string)) {
      setWarningMessage("Solo administradores pueden guardar caminos.");
      setWarningModalOpen(true);
      return;
    }
  ...
  const handleDeletePath = async (pathId: number | undefined) => {
    if (!pathId || !["admin", "superuser"].includes(role as string)) {
      setWarningMessage("Solo administradores pueden eliminar caminos.");
      setWarningModalOpen(true);
      return;
    }
